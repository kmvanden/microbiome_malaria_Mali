...
dds = DESeqDataSetFromMatrix(countData = otu_med,
                              colData = metadata,
                              design = ~ outcome)
...
sum(resOrdered$padj < 0.05) # 43 differentially abundant taxa (padj < 0.05)
sum(resOrdered$padj < 0.10) # 72 differentially abundant taxa (padj < 0.10)
...
plotCounts(dds, gene = "Lachnoclostridium_sp._YL32", intgroup = "outcome")
plotCounts(dds, gene = "Clostridium_sp._M62/1", intgroup = "outcome")
plotCounts(dds, gene = "Blautia_argi", intgroup = "outcome")
plotCounts(dds, gene = "Prevotella_copri", intgroup = "outcome")
plotCounts(dds, gene = "Streptococcus_thermophilus", intgroup = "outcome")
plotCounts(dds, gene = "Veillonella_parvula", intgroup = "outcome")


### EXPORTING DATA | fold change and padj values
# exporting log2 fold change and padj to csv files 
write.csv(as.data.frame(resOrdered), file = "bracken_deseq2_fold_change_padj.csv")


### EXPORTING DATA | normalized counts 
dds = estimateSizeFactors(dds)
ddsCounts = counts(dds, normalized = TRUE)
write.csv(as.data.frame(ddsCounts), file = "bracken_deseq2_counts.csv")
...
otu_med = otu_table(otu_med, taxa_are_rows = TRUE)
...
phylo = phyloseq(otu_med, metadata)
...
corncob = bbdml(formula = Blautia_argi ~ outcome,
                phi.formula = ~ outcome,
                data = phylo)
...
# list of all significant taxa (78 at padj < 0.10)
...
corncob_padj010 = corncob_df(corncob_all010) %>%
  filter(fdr < 0.10) %>%
  filter(!grepl("Intercept", covariate))
write.csv(as.data.frame(corncob_padj010), file = "corncob_padj010.csv")
...
fit_data010 = Maaslin2(input_data = otu_med, input_metadata = metadata, output = "maaslin2_output010",
...
otu_med_tss = normalizeFeatures(features = otu_med, normalization = 'TSS') # TSS normalization
otu_med_tss_log = transformFeatures(features = otu_med_tss, transformation = 'LOG') # LOG transformation
write.csv(as.data.frame(otu_med_tss_log), file = "maaslin2_tss_log_counts.csv")
...
write.csv(as.data.frame(ald), file = "bracken_aldex2_effect_padj.csv")

ald_clr = aldex.clr(otu_med, metadata, mc.samples = 128)
ald_clr = getMonteCarloInstances(ald_clr)

write.csv(as.data.frame(ald_clr), file = "bracken_aldex2_clr_counts.csv")
...
# [33] ashr_2.2-63                 apeglm_1.16.0               DESeq2_1.34.0               SummarizedExperiment_1.24.0
# [37] Biobase_2.58.0              MatrixGenerics_1.6.0        matrixStats_0.60.1          GenomicRanges_1.44.0       
# [41] GenomeInfoDb_1.30.1         IRanges_2.26.0              S4Vectors_0.30.2            BiocGenerics_0.38.0        
# [45] phyloseq_1.42.0            
# 
...
# [49] emdbook_1.3.13           mclust_5.4.7             bit_4.0.4                proxy_0.4-26            
