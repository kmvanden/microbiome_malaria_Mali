...
sum(colnames(otu)==row.names(metadata))
...
# 7 differentially abundant taxa (padj < 0.05)
sum((resOrdered$padj < 0.05)[!is.na((resOrdered$padj < 0.05))])
# 21 differentially abundant taxa (padj < 0.10)
sum((resOrdered$padj < 0.10)[!is.na((resOrdered$padj < 0.10))])
...
plotMA(resLFC_normal, main="normal", ylim = c(-5,5))

# adaptive t prior shrinkage estimator (from apeglm package)
resLFC_apeglm <- lfcShrink(dds, coef=2, type="apeglm")
plotMA(resLFC_apeglm, main="apeglm", ylim=c(-5,5))

# adaptive shrinkage estimator (from ashr package)
resLFC_ashr <- lfcShrink(dds, coef=2, type="ashr")
plotMA(resLFC_ashr, main="ashr", ylim=c(-5,5))


### INDIVIDUAL PLOTS
# normalizes counts by estimated size factors + adds pseudocount of 1/2 for log scale plotting
plotCounts(dds, gene = "Otu001344", intgroup = "outcome2")
plotCounts(dds, gene = "Otu000056", intgroup = "outcome2")
plotCounts(dds, gene = "Otu001234", intgroup = "outcome2")
plotCounts(dds, gene = "Otu000040", intgroup = "outcome2")
plotCounts(dds, gene = "Otu000083", intgroup = "outcome2")
plotCounts(dds, gene = "Otu000008", intgroup = "outcome2")
plotCounts(dds, gene = "Otu000217", intgroup = "outcome2")


### EXPORTING DATA | fold change and padj values
write.csv(as.data.frame(resOrdered), file = "human16S_deseq2_fold_change_padj.csv")

### EXPORTING DATA | normalized counts
dds <- estimateSizeFactors(dds)
ddsCounts <- counts(dds, normalized = TRUE)
write.csv(as.data.frame(ddsCounts), file = "human16S_deseq2_counts.csv")
...
# fit our model to a single taxa with the covariate "outcome2"
corncob = bbdml(formula = Otu000056 ~ outcome2,
                 phi.formula = ~ outcome2,
                 data = phylo)

# plot data on a relative abundance scale 
plot(corncob, total = TRUE, B = 1000, color = "outcome2")
...
corncob_all005 = differentialTest(formula = ~ outcome2,
                                  phi.formula = ~ outcome2,
                                  formula_null = ~ 1,
                                  phi.formula_null = ~ outcome2,
                                  test = "Wald", boot = FALSE,
                                  data = phylo,
                                  fdr_cutoff = 0.05)

# list of significant taxa (17 at padj < 0.05)
...
# plot the model coefficients of out results for significant taxa
plot(corncob_all005)

# test and plot all taxa at a padj of < 0.10
corncob_all010 = differentialTest(formula = ~ outcome2,
                                  phi.formula = ~ outcome2,
                                  formula_null = ~ 1,
                                  phi.formula_null = ~ outcome2,
                                  test = "Wald", boot = FALSE,
...
# list of significant taxa (22 at padj < 0.10)
...
write.csv(as.data.frame(corncob_padj005), file = "human16S_corncob_padj005.csv")

# use the function to filter taxa with padj < 0.10
corncob_padj010 <- corncob_df(corncob_all010) %>%
  filter(fdr < 0.10) %>%
  filter(!grepl("Intercept", covariate))
write.csv(as.data.frame(corncob_padj010), file = "human16S_corncob_padj010.csv")
...
write.csv(as.data.frame(otu_tss_log), file = "human16S_maaslin2_tss_log_counts.csv")
...
# convert metadata into a vector of the outcome2 group
metadata = metadata$outcome2
...
write.csv(as.data.frame(ald), file = "human16S_aldex2_effect_padj.csv")

ald_clr = aldex.clr(otu, metadata, mc.samples = 128)
ald_clr = getMonteCarloInstances(ald_clr)

write.csv(as.data.frame(ald_clr), file = "human16S_aldex2_clr_counts.csv")
...
#   [1] ALDEx2_1.26.0               zCompositions_1.4.0-1       truncnorm_1.0-8            
# [4] NADA_1.6-1.1                survival_3.2-13             MASS_7.3-54                
# [7] edgeR_3.36.0                metagenomeSeq_1.36.0        RColorBrewer_1.1-2         
# [10] glmnet_4.1-2                Matrix_1.5-4                limma_3.50.3               
# [13] car_3.0-12                  carData_3.0-4               chemometrics_1.4.4         
# [16] rpart_4.1-15                vegan_2.6-4                 lattice_0.20-44            
# [19] permute_0.9-7               Maaslin2_1.8.0              lubridate_1.9.2            
...
# [31] janitor_2.2.0               corncob_0.3.1               phyloseq_1.42.0            
# [34] ashr_2.2-63                 apeglm_1.16.0               DESeq2_1.34.0              
# [37] SummarizedExperiment_1.24.0 Biobase_2.58.0              MatrixGenerics_1.6.0       
# [40] matrixStats_0.60.1          GenomicRanges_1.44.0        GenomeInfoDb_1.30.1        
# [43] IRanges_2.26.0              S4Vectors_0.30.2            BiocGenerics_0.38.0        
# 
